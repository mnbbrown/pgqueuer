...
class Queries:
    ...
    async def upgrade(self) -> None:
        ...
        Existing indexes are pre-fetched so that CREATE INDEX statements
        are skipped for indexes that already exist. This prevents the
        ShareLock that CREATE INDEX IF NOT EXISTS acquires even when the
        index is already present, which can deadlock with concurrent
        INSERT operations from workers.
        """

        # Pre-fetch existing indexes to avoid ShareLock deadlocks.
        ...
        for query in self.qbe.build_upgrade_queries():
            m = self._CREATE_INDEX_RE.match(query.strip())
    ...
    async def log_jobs(
        ...
        Args:
            job_status (list[tuple[models.JobId, models.JOB_STATUS]]): A list of tuples
                containing job IDs and their corresponding statuses
